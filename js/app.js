...
/* =========================================================================
   5) EXPORTAÇÃO EM PDF — captura BLOCO A BLOCO (não o relatório inteiro de
   uma vez). Um único print gigante de um elemento fora da tela e muito alto
   pode sair corrompido/rasgado em alguns navegadores (limite de textura da
   GPU); capturando cada bloco pequeno separadamente esse risco desaparece,
   e como bônus os cortes de página nunca caem no meio de uma frase.
   ========================================================================= */
function buildReportBlocks(nome, idade, queixa, objetivo, riscos, obs) {
  const blocks = [];
  ...
  ultimoResultado.top3.forEach(k => {
  ...
  blocks.push(`<div class="block"><h3>Observações e combinações</h3><ul><li>(preencher em sessão)</li></ul></div>`);
  return blocks;
}
...
async function exportPdf() {
  ...
  const btn = $('#btn-pdf'); if (btn) btn.disabled = true;
  ...
  try {
    ...
    const A4_W = 794, A4_H = 1123;   // px de referência (96dpi) usados para o layout de cada bloco
    const MARGIN = 24;                // pt
    const FOOTER_H = 16;              // pt reservados para nome/data no rodapé de cada página
    const px2pt = px => px * 0.75;
    const pageW = px2pt(A4_W), pageH = px2pt(A4_H);
    const contentW = pageW - MARGIN * 2;
    const scale = 2; // fixo — nitidez suficiente sem arriscar canvas gigante

    const mount = document.createElement('div');
    mount.style.position = 'fixed';
    mount.style.left = '-10000px';
    mount.style.top = '0';
    mount.style.width = A4_W + 'px';
    document.body.appendChild(mount);

    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF('p', 'pt', 'a4');
    let y = MARGIN, firstPage = true;

    const blocks = buildReportBlocks(nome, idade, queixa, objetivo, riscos, obs);

    for (const html of blocks) {
      const box = document.createElement('div');
      box.className = 'print-wrap';
      box.style.width = A4_W + 'px';
      box.innerHTML = html;
      mount.innerHTML = '';
      mount.appendChild(box);

      // Cada bloco é pequeno (uma técnica, uma semana) — captura segura, sem
      // risco de estourar limite de textura da GPU nem gerar imagem corrompida.
      const canvas = await html2canvas(box, {
        scale, backgroundColor: '#fff', useCORS: true, foreignObjectRendering: false
      });

      const imgWpt = contentW;
      const imgHpt = round2(imgWpt * (canvas.height / canvas.width));
      const maxUsableH = pageH - MARGIN - FOOTER_H;

      if (!firstPage && y + imgHpt > pageH - FOOTER_H) {
        pdf.addPage();
        y = MARGIN;
      }
      firstPage = false;

      // Bloco maior que uma página inteira (raro): ainda assim desenha,
      // começando do topo de uma página nova, em melhor esforço.
      if (imgHpt > maxUsableH && y > MARGIN) {
        pdf.addPage();
        y = MARGIN;
      }

      const url = canvas.toDataURL('image/png');
      pdf.addImage(url, 'PNG', MARGIN, y, imgWpt, imgHpt);
      y += imgHpt + 10;
    }

    // Rodapé (nome + data) em todas as páginas geradas.
    const totalPages = pdf.internal.getNumberOfPages();
    for (let i = 1; i <= totalPages; i++) {
      pdf.setPage(i);
      pdf.setFontSize(9); pdf.setTextColor(68, 104, 108);
      pdf.text(nome, MARGIN, pageH - 8);
      const tw = pdf.getTextWidth(dh);
      pdf.text(dh, pageW - MARGIN - tw, pageH - 8);
    }
