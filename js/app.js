...
const GUIDES = [
 {key:'dbt', title:'DBT – STOP + TIPP (Crise/alta ativação)', when:'Intensidade ≥7/10; impulso alto.', steps:[
  ...
  'Checagem de intensidade e ancoragem (ação segura de 10 min).'
 ...
 {key:'cft', title:'CFT – Postura/voz compassiva', when:'Vergonha, rigidez, “tenho que”.', steps:[
  ...
  'Mapear: Crítico diz __ → Resposta compassiva __.',
  'Âncora: “Eu vejo. Eu fico. Próximo passo possível: ___.”',
  'Defina micro-ação de 5–10 min.'
 ...
 {key:'gestalt', title:'Gestalt – Cadeira interna', when:'Ambivalência/evitação relacional.', steps:[
  'Nomeie partes (Crítico × Vulnerável; Controlador × Espontâneo).',
  'Troca de cadeiras: 2–3 falas por parte.',
  'Integração: o que cada parte precisa para 1 passo?',
  'Ação final: pedido/limite/experimento concreto.'
 ], script:'“Da cadeira do Crítico: ‘Se eu te protejo, ganho o quê?’… Troca… ‘Hoje, o passo é ___.’”'},
 {key:'fap', title:'FAP – Coragem relacional aqui-agora', when:'Dificuldade em pedir/discordar.', steps:[
  'Evocar o comportamento na sessão (“como diria não para mim?”).',
  'Notar topografia/função (esquiva).',
  'Modelar duas frases de pedido direto.',
  'Reforçar natural (“quando foi claro, eu me aproximei”).',
 ...
 {key:'abc', title:'Análise Comportamental – Exposição graduada', when:'Medo mantido por fuga/rituais.', steps:[
  ...
  'Sem neutralizar (sem checar/buscar garantias).',
  'Reforço imediato pós-execução.',
...
// Gerar protocolo a partir da anamnese
$('#btn-gerar')?.addEventListener('click', ()=>{
  ...
  if (/falar|pedido|limite|relacion|conversa/i.test(objetivo)) score.habilidades+=1;
...
// ===== PDF robusto com espaçamento (html2canvas + jsPDF) =====
document.getElementById('btn-pdf')?.addEventListener('click', gerarPDFRobusto);
async function gerarPDFRobusto(){
  ...
  const funcao = window.estado?.anamnese?.funcao || '—';
  const gatilhos = window.estado?.anamnese?.gatilho || '—';

  // build print view
  const wrap = document.createElement('div'); wrap.className='print-wrap';
  wrap.innerHTML = `
    <h2>Parecer Clínico – Mentor Humanista</h2>
    <div class="meta">Nome: <b>${esc(nome)}</b> • Data: ${hoje}<br>Queixa: ${esc(queixa)}<br>Objetivo: ${esc(objetivo)}</div>

    <div class="block">
      <h3>Formulação breve</h3>
      <ul>
        <li>Função predominante: ${esc(funcao)}.</li>
        <li>Gatilhos principais: ${esc(gatilhos)}.</li>
    ...
    <div class="block">
      <h3>Metas operacionais (4 semanas)</h3>
      <ul>
        <li>Semana 1: sobreviver a crises + rotina mínima de sono.</li>
        <li>Semana 2: regulação/contato (CFT/experimentos).</li>
        <li>Semana 3: habilidade relacional ou exposição graduada.</li>
    ...
    <div class="block">
      <h3>Observações e combinações</h3>
      <ul><li>(preencher)</li></ul>
    </div>`;
  ...
  if (imgH <= pageH - margin*2) {
    ...
    let sY = 0;
    const pagePx = (pageH - margin*2) * (canvas.width / imgW);
    while (sY < canvas.height) {
      ...
      const partH = (sliceH / canvas.width) * imgW;
      pdf.addImage(partData, 'PNG', margin, margin, imgW, partH, '', 'FAST');
      sY += sliceH;
